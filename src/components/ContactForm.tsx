...
import { getSavedRegionName } from './RegionLocator';
...
export function ContactForm(){
 ...
 useEffect(()=>{
  ...
  const city=(params.get('city')||getSavedRegionName()||'').slice(0,100);
  if(cityRef.current&&!cityRef.current.value)cityRef.current.value=city;
  const update=(event:Event)=>{const name=(event as CustomEvent<{name?:string}>).detail?.name;if(cityRef.current&&!cityRef.current.value&&name)cityRef.current.value=name;};
  window.addEventListener('saarmontage-region',update);return()=>window.removeEventListener('saarmontage-region',update);
 },[]);
