...
const serviceSlugs=['moebelmontage','ikea-moebelmontage','pax-montage','kuechenmontage','schrankmontage','bettmontage','bueromoebelmontage'];
...
export function Header() {
  ...
  return <header ref={headerRef} className={`site-header sm-header ${scrolled ? 'is-scrolled' : ''}`} onKeyDown={event=>{if(event.key==='Escape'){setOpen(false);menuRef.current?.focus();}}}><div className="container header-inner sm-header-inner"><Brand/><nav id="main-navigation" aria-label="Hauptnavigation" className={`main-navigation sm-nav${open?' is-open':''}`} onClick={()=>setOpen(false)}>{navigation.map(item=>{const current=pathname===item.href||(item.href==='/leistungen/'&&serviceSlugs.some(slug=>pathname===`/${slug}/`));return <a key={item.href} href={item.href} aria-current={current?'page':undefined}>{item.label}</a>})}</nav><div className="header-actions sm-header-actions"><WhatsAppButton className="header-cta sm-header-cta">WhatsApp</WhatsAppButton><button ref={menuRef} type="button" className="menu-button sm-menu-toggle" aria-label={open?'Menü schließen':'Menü öffnen'} aria-expanded={open} aria-controls="main-navigation" onClick={()=>setOpen(!open)}><span className="sm-menu-bars" aria-hidden="true"><i/><i/><i/></span></button></div></div></header>;
}
