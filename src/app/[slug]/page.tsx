...
export async function generateMetadata({params}:Props):Promise<Metadata>{
 ...
 const description=service?.description||(region?`Möbelmontage in ${region.name}: Stadtteile, Anfahrt, Projektbeispiel und persönliche Anfrage.`:`${route.title}. Persönlicher Montageservice von Andrii Ryndia Solutions im Saarland.`);
 return {title:route.title,description,alternates:{canonical:`/${slug}/`},openGraph:{title:route.title,description,url:`/${slug}/`,siteName:'Saarmontage · Andrii Ryndia Solutions',locale:'de_DE',type:'website',images:[{url:'/opengraph-image',width:1200,height:630,alt:'Saarmontage – Möbel- und Küchenmontage im Saarland'}]},twitter:{card:'summary_large_image',title:route.title,description,images:['/opengraph-image']}};
}
