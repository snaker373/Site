(function () {
  ...
  function setSubmitLoading(btn, loading) {
    ...
    btn.innerHTML = loading
      ? '<i data-lucide="loader-circle"></i>Wird gesendet ?'
      : '<i data-lucide="send"></i>Anfrage senden';
    initIcons();
  }
  ...
  function initContactForm() {
    ...
    var contactMessage = 'Bitte geben Sie eine Telefonnummer oder E-Mail-Adresse an.';

    function validateContactMethod() {
      if (!phoneField || !emailField) return true;
      var hasContact = phoneField.value.trim() || emailField.value.trim();
      phoneField.setCustomValidity(hasContact ? '' : contactMessage);
    ...
    if (phoneField && emailField) {
      ...
      phoneField.addEventListener('blur', validateContactMethod);
      emailField.addEventListener('blur', validateContactMethod);
    }
    ...
    form.addEventListener('submit', function (event) {
      ...
      fetch('https://api.web3forms.com/submit', {
        ...
        .catch(function () {
          alert('Verbindungsfehler. Bitte versuchen Sie es sp?ter erneut oder schreiben Sie uns per WhatsApp.');
          setSubmitLoading(btn, false);
        });
    });
  }
  ...
  function initCookieBanner() {
    ...
    function setCookieConsent(value) {
      try {
        ...
        // If storage is unavailable, still dismiss the banner for this page view.
      }
    }
